...
class HTMLCleaner {
    ...
    var $CleanUpTags = array(//array of inline tags that can be merged
        'a','span','b','i','u','strong','em',
        'big','small','tt','var','code','xmp','cite','pre',
        'abbr','acronym','address','q','samp','sub','sup'
    );    var $TidyConfig;
    var $Encoding = 'latin1';
    var $Version = '1.0 RC6';

    function HTMLCleaner(){
        $this->Tag_whitelist =  join($this->Tag_whitelist);
        $this->Options =  array(
    ...
    function RemoveBlacklistedAttributes($attribs){
        ...
        $this->html =   preg_replace('/[\s]+('.$attribs.')[\s]*=[\s]*("[^"]*"|\'[^\']*\')/m',"",$this->html); //double and single quoted
        $this->html =   preg_replace('/[\s]+('.$attribs.')[\s]*=[\s]*[^ |^>]*/mi',"",$this->html);  //not quoted
    }
            ...
            if((int)phpversion() >=  5){
                ...
                while($repl){
                    ...
                    foreach($this->CleanUpTags as $tag){
                        $this->html = preg_replace("/<($tag)[^>]*>[\s]*([(&nbsp;)]*)[\s]*<\/($tag)>/i","\\2", $this->html,-1,$count);
                        //strip empty inline tags (must be on top of merge inline tags)
                        $repl += $count;
                        $this->html = preg_replace("/<\/($tag)[^>]*>[\s]*([(&nbsp;)]*)[\s]*<($tag)>/i","\\2", $this->html,-1,$count);
                        //merge inline tags
                        $repl += $count;
